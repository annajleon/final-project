...
Below is a Histogram of the average charge on the x axis and the number of hosts is on the y axis. I removed outliers above an average charge of $1000 to better see the distribution in the majority of the data.
...
Below is a scatterplot of the top five most expensive neighborhoods for Airbnbs in NYC. I used size of the points to represent count, how many Airbnbs there are in that neighborhood, so that we could get an idea of how many Airbnb price points wennt into the calculation of that neighborhood's average price. I also used a factor to order the x axis so that the neighborhoods were in decreasing order by average price. 
...
Below is a scatterplot of all of the neighborhoods this time. I faceted the graph by borough and also shaped the points by borough as well to ensure that viewers could easily tell which points belonged to which borough. It is clear that Manhattan's neighborhoods are generally pricier than the other boroughs.
...
The following boxplot shows the price distribution of listings, faceted by borough. I removed some of the outliers by adjusting the y axis so that it only went up to 750. Note that all boroughs had a few upper price point Airbnbs removed from the plot, so that the majority of price points were easier to see and analyze.
...
The boxplot below is the same plot as above, except not faceted by borough, so it is just for the general all listings in general. I again limited the y axis to (0,750).
...
This faceted bar chart shows the number of Airbnbs that are in the various price ranges per borough. For whatever reason, we can determine that Queens has a ton of Airbnbs in the moderate range, close to its average price, and very few that are much cheaper than that.
